...
- Logo: `<img src="boot-logo.png" width="104" height="104">` — a transparent-bg
  208×208 PNG generated by `scripts/build-boot-logo.js` from the same wallet
  SVG used in the native master. **Do NOT use `icon-192.png`** — that's the
  PWA launcher icon with white square padding designed for iOS rounded corner
  / Android adaptive icon masks; against the dark splash background the white
  padding shows as a frame artifact.
